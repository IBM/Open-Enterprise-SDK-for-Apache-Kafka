      ...
      *---------------------------------------------------------------
      *  Pointers to the different Kafka Topics
      *---------------------------------------------------------------
       01 KAFKA-TOPIC-REF1 USAGE POINTER.
       01 KAFKA-TOPIC-REF2 USAGE POINTER.
       01 KAFKA-TOPIC-REF3 USAGE POINTER.
       01 KAFKA-TOPIC-REF4 USAGE POINTER.
       01 KAFKA-TOPIC-REF5 USAGE POINTER.
       01 KAFKA-TYPE-REF1 USAGE POINTER.
       01 KAFKA-TYPE-REF2 USAGE POINTER.
       01 KAFKA-TYPE-REF3 USAGE POINTER.
       01 KAFKA-TYPE-REF4 USAGE POINTER.
       01 KAFKA-TYPE-REF5 USAGE POINTER.
      ...
      *---------------------------------------------------------------
      * If we are working with TOPIC 1, set kafka-type-ref1 to the
      * address returned by the function IXY-KAFKA-NEW
      *
      * If working with TOPIC 2 set kafka-type-ref2 to the address
      * returned by the function IXY-KAFKA-NEW
      *---------------------------------------------------------------
                IF KAFKA-TOPIC-1
                   SET KAFKA-TYPE-REF1 TO KAFKA-TYPE-REF
                      OF KAFKA-NEW-OUT
                   SET KAFKA-TOPIC-REF1 TO KAFKA-TOPIC-REF
                      OF KAFKA-TOPIC-NEW-OUT
                ELSE
                   IF KAFKA-TOPIC-2
                      SET KAFKA-TYPE-REF2 TO KAFKA-TYPE-REF
                         OF KAFKA-NEW-OUT
                      SET KAFKA-TOPIC-REF2 TO KAFKA-TOPIC-REF
                         OF KAFKA-TOPIC-NEW-OUT
                   ELSE
                      IF KAFKA-TOPIC-3
                         SET KAFKA-TYPE-REF3 TO KAFKA-TYPE-REF
                            OF KAFKA-NEW-OUT
                         SET KAFKA-TOPIC-REF3 TO KAFKA-TOPIC-REF
                            OF KAFKA-TOPIC-NEW-OUT
                      ELSE
                         IF KAFKA-TOPIC-4
                            SET KAFKA-TYPE-REF4 TO KAFKA-TYPE-REF
                               OF KAFKA-NEW-OUT
                            SET KAFKA-TOPIC-REF4 TO KAFKA-TOPIC-REF
                               OF KAFKA-TOPIC-NEW-OUT
                         ELSE
                            SET KAFKA-TYPE-REF5 TO KAFKA-TYPE-REF
                               OF KAFKA-NEW-OUT
                            SET KAFKA-TOPIC-REF5 TO KAFKA-TOPIC-REF
                               OF KAFKA-TOPIC-NEW-OUT
                         END-IF
                      END-IF
                   END-IF
                ...
                DISPLAY '+-- KAFKA-NEW - KAFKA-TYPE-REF1 -'
                        KAFKA-TYPE-REF1
                DISPLAY '+-- KAFKA-NEW - KAFKA-TYPE-REF2 -'
                        KAFKA-TYPE-REF2
                DISPLAY '+-- KAFKA-NEW - KAFKA-TYPE-REF3 -'
                        KAFKA-TYPE-REF3
                DISPLAY '+-- KAFKA-NEW - KAFKA-TYPE-REF4 -'
                        KAFKA-TYPE-REF4
                DISPLAY '+-- KAFKA-NEW - KAFKA-TYPE-REF5 -'
                        KAFKA-TYPE-REF5
                DISPLAY '+-- KAFKA-NEW - KAFKA-TOPIC-REF1 -'
                        KAFKA-TOPIC-REF1
                DISPLAY '+-- KAFKA-NEW - KAFKA-TOPIC-REF2 -'
                        KAFKA-TOPIC-REF2
                DISPLAY '+-- KAFKA-NEW - KAFKA-TOPIC-REF3 -'
                        KAFKA-TOPIC-REF3
                DISPLAY '+-- KAFKA-NEW - KAFKA-TOPIC-REF4 -'
                        KAFKA-TOPIC-REF4
                DISPLAY '+-- KAFKA-NEW - KAFKA-TOPIC-REF5 -'
                        KAFKA-TOPIC-REF5

                DISPLAY ' CREATE NEW TOPIC NUMBER - ' KAFKA-TOPIC-NBR

      * Create PRODUCE
           WHEN KAFKA-PRODUCE
      *---------------------------------------------------------------
      * If we are working with TOPIC 1, we set the Topic to send to
      * the address of KAFKA-TOPIC-REF1
      *
      * If we are working with TOPIC 2, we set the Topic to send to
      * the address of KAFKA-TOPIC-REF2
      *---------------------------------------------------------------

                IF KAFKA-TOPIC-1
                   SET KAFKA-TYPE-REF OF KAFKA-PRODUCEV-IN TO
                      KAFKA-TYPE-REF1
                   SET KAFKA-TOPIC-REF OF KAFKA-PRODUCEV-IN
                      TO KAFKA-TOPIC-REF1
      ...
      *                      TO KAFKA-TOPIC-REF OF KAFKA-TOPIC-NEW-OUT
                ...
                MOVE PARTITION-VALUE TO PARTITION OF KAFKA-PRODUCEV-IN
                MOVE MSGFLAGS-VALUE TO MSGFLAGS OF KAFKA-PRODUCEV-IN
      ...
      * Convert Kafka payload into ASCII
                IF SKIP-CONV = 'Y'
                   IF CALLER-31BIT = 'Y'
                      SET PAYLOAD OF KAFKA-PRODUCEV-IN TO
                         KAFKA-PAYLOAD-31
                   ELSE
                      SET PAYLOAD OF KAFKA-PRODUCEV-IN TO
                         KAFKA-PAYLOAD
                ...
                ELSE
                   ...
                   PERFORM CONVERT-EBC-ASC
                   SET PAYLOAD OF KAFKA-PRODUCEV-IN
                      TO ASCII-DATA-PTR-31 OF EBCDIC-ASCII-CONV-OUT
                END-IF

                SET PROD-KEY OF KAFKA-PRODUCEV-IN TO NULL
                MOVE 0 TO KEY-LEN OF KAFKA-PRODUCEV-IN
                SET MSG-OPAQUE OF KAFKA-PRODUCEV-IN TO NULL
                DISPLAY "PRODUCEV CALLED"

                MOVE FUNCTION IXY-KAFKA-PRODUCEV(
                   KAFKA-TYPE-REF OF KAFKA-PRODUCEV-IN
                   KAFKA-TOPIC-REF OF KAFKA-PRODUCEV-IN
                   PARTITION OF KAFKA-PRODUCEV-IN
                   MSGFLAGS OF KAFKA-PRODUCEV-IN
                   PAYLOAD OF KAFKA-PRODUCEV-IN
                   PAYLOAD-LEN OF KAFKA-PRODUCEV-IN
                   PROD-KEY OF KAFKA-PRODUCEV-IN
                   KEY-LEN OF KAFKA-PRODUCEV-IN
                   MSG-OPAQUE OF KAFKA-PRODUCEV-IN
                   )
                   TO RETURN-STATUS OF KAFKA-PRODUCEV-OUT

                IF RETURN-STATUS OF KAFKA-PRODUCEV-OUT NOT = 0
      ...
      *---------------------------------------------------------------
                IF KAFKA-TOPIC-1
                   SET KAFKA-TYPE-REF OF KAFKA-FLUSH-IN TO
                      KAFKA-TYPE-REF1
                   SET KAFKA-TOPIC-REF OF KAFKA-TOPIC-DESTROY-IN
                      TO KAFKA-TOPIC-REF1
                ...
                ELSE
                   ...
                   ELSE
                      ...
                      ELSE
                         IF KAFKA-TOPIC-4
                            SET KAFKA-TYPE-REF OF KAFKA-FLUSH-IN TO
                               KAFKA-TYPE-REF4
                            SET KAFKA-TOPIC-REF OF
                               KAFKA-TOPIC-DESTROY-IN
                               TO KAFKA-TOPIC-REF4
                            SET KAFKA-TYPE-REF OF KAFKA-DESTROY-IN TO
                               KAFKA-TYPE-REF4
                         ELSE
                   ...
                   END-IF
                END-IF
                DISPLAY '*-- KAFKA-TOPIC-NBR --* ' KAFKA-TOPIC-NBR
                DISPLAY '*-- KAFKA-TOPIC-REF --* '
                        KAFKA-TOPIC-REF
                   OF KAFKA-TOPIC-DESTROY-IN
      * Flush the queue
      *      SET KAFKA-TYPE-REF OF KAFKA-FLUSH-IN TO
      *                      KAFKA-TYPE-REF OF KAFKA-NEW-OUT
